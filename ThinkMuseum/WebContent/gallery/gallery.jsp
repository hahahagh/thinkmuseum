...
<%
	if(list != null && list.size() > 0){
		for(Painting painting : list){
			%>
		<div style="width: 320px; height: 450px; float: left; margin: 40px; background-color: #444444" >
			<a href="content.jsp?num=<%=painting.getNum()%>&pageNum=<%=pageNum%>">
			<img src="../upload/<%=painting.getFilename()%>" class="img" style="width: 100%; height: 80%; margin-bottom: 15px;">
			</a>
		<div style="width: 320px; background-color: #444444; color:  #A6A6A6;">
			<div style="margin-bottom: 10px;">
			<b style="color:#F6F6F6;">&nbsp&nbsp &lt <%=painting.getTitle()%> &gt </b> 
			</div>		
			<div style="margin-bottom: 20px;">
			&nbsp&nbsp- <%=painting.getArtist()%>
			</div>	
		</div>		
		</div>	
			<%
